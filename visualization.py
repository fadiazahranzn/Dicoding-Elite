...
def create_funnel_chart(dataframe: pd.DataFrame, column: str, label: str, color: Optional[str] = None) -> Figure:
    """
    ...
    - color: Optional[str] - The column to color the funnel chart by.
    ...
    if color:
        ...
        column_counts.columns = [label, 'Count']
        fig = px.funnel(column_counts, y=label, x='Count', title=f"{label} Proportion")
        fig.update_traces(hovertemplate='<b>%{label}</b><br>Total: %{value} person.<extra></extra>')
    return fig
